...
\usepackage{parskip} % For increased paragraph spacing
...
\usepackage{bbm} % Extra mathbb fonts
\usepackage{dsfont} % Extra mathds fonts
\usepackage{array} % Enables array features
\usepackage[binary-units=true, per-mode=symbol]{siunitx} % For SI Unit easy formatting
\sisetup{detect-all}
...
\usepackage{adjustbox} % sev­eral macros to ad­just boxed con­tent
...
\usepackage{bm} % For bold table entries with bftab
\newcommand{\bftab}{\fontseries{b}\selectfont} % Command to bold one cell in a table
...
% =============== Other ===============
\usepackage{soul} % For striking and highlighting
