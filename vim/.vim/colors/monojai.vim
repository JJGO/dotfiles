...
let g:colors_name = "monojai"

" User Options
if !exists("g:monojai_italic")
    let g:monojai_italic = 0
endif
...
hi def link goArgumentName      SublimeContextParam
hi def link goDeclType          SublimeType
hi def link goDeclaration       SublimeType
hi def link goField             Identifier
hi def link goFunction          Tag
hi def link goFunctionCall      SublimeFunctionCall
...
hi def link goGenerate          Comment
hi def link goGenerateVariables Comment
" Variation: It's nice to have builtins highlighted specially, though Sublime
" doesn't do this. I would use `Special` here.
hi def link goExtraType         Identifier
hi def link goImport            Keyword
hi def link goPackage           Keyword
hi def link goReceiverVar       SublimeContextParam
hi def link goStatement         Keyword
hi def link goType              SublimeType
" Variation: I like this better as `SublimeType`, since it has symmetry with
" `goType`.
hi def link goTypeConstructor   Identifier
hi def link goTypeDecl          SublimeType
hi def link goTypeName          Tag
